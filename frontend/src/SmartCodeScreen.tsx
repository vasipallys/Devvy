...
import { api } from './api'
import type { SmartCodePreview, SmartCodeRequest } from './types'
...
export function SmartCodeScreen({ onHome }: { onHome: () => void }) {
  ...
  async function run() {
    ...
    setError(''); setPreview(undefined); setResult(undefined); setStages([]); setRunning(true)
  ...
  async function apply() {
    ...
    try { setResult(await api.smartCodeApply(preview.preview_token)); setStatus('Changes applied and evidence saved') }
    catch (cause) { setError((cause as Error).message) }
    finally { setApplying(false) }
  }

  return <div className="product-screen smart-screen">
    <header className="product-header"><button onClick={onHome}><ArrowLeft size={17}/> Home</button><div className="product-brand"><Code2/><span><b>Smart Code</b><small>Repository-aware engineering agent</small></span></div><div className="product-model"><span className="pulse"/> Gemma · local</div></header>
    <div className="smart-layout">
      <aside className="smart-controls">
        ...
        {targets.length > 0 && <div className="target-list">{targets.map(path => <span key={path} title={path}>{path.split(/[\\/]/).pop()}<button onClick={() => setTargets(items => items.filter(item => item !== path))}>×</button></span>)}</div>}
