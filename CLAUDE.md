...
cd frontend; npm test                                # vitest (jsdom) — the chat sanitiser
```
...
- **`agent_graph.py` `TalkAgentGraph`** — Talk's voice companion, **YUKTI**: an executive AI butler and second-brain keeper. Graph: `gate → route_visual → (recall →)? (research →)? companion`. Keyword sets decide `requires_research` (news/weather/current), `requires_recall` (second-brain terms), `requires_briefing`, and `requires_animation` (math/visual terms, which trigger a Manim render), and also produce a `route_reason`. Recall runs before research — the user's own notes are cheaper, more specific and more trustworthy than the web.

**`yukti.py`** holds the persona, the capability register and the voice shaping. It carries `GROUNDING_CONTRACT_VOICE`, the third form of the contract — **never the extraction form**. "Use only the facts directly stated above; if the information is missing say exactly *The provided text does not contain this information*" is right for scoring a story and catastrophic for a companion: asked for the news, it recites the refusal, because most questions asked out loud are not answerable from the prompt's own text. The voice form keeps every rule about invention and points it at what is dangerous to invent — live facts and facts about the user — while explicitly permitting general knowledge and ordinary conversation. The register is the load-bearing part and it is **data, not prose**: each faculty records whether it is actually wired, and an unwired one carries the exact sentence YUKTI says instead. Five are wired (second brain, memory, web research, documents, animation); four are not (screen vision, calendar, email, model swapping). `gate` refuses those **in code, before generation** — a prompt telling a 1B model not to describe a screen it cannot see still lets it describe one perhaps once in five, and spoken aloud there is no URL to click and no panel to open, so the listener has nothing to check it against. Do not add a faculty to the persona without wiring it.

`speakable()` is the other half of the same idea. Everything YUKTI produces is read by pyttsx3, which pronounces Markdown as punctuation and a URL one character at a time, so **the screen gets the rich text and the synthesiser gets prose**. It is idempotent and leaves plain prose untouched; `voice_chunks()` splits on sentences but never after a bare number, which is a list marker.

**`second_brain.py`** is two stores that fail differently. The **notes vault** (`YUKTI_VAULT_ROOT`) is the user's own files: read-only, ranked by term frequency with the filename weighted heavily, tooling directories skipped, and traversal **refused rather than sanitised** — quietly rewriting a path answers a question from a file nobody meant. The **memory bank** (migration 4) is YUKTI's own: written only on an explicit "remember that…" / "call me…", never on inference, because a memory the user did not state gets recalled months later with all the authority of one they did. Each entry keeps its source sentence; a newer statement of the same kind supersedes; standing preferences are recalled every turn regardless of word overlap. A vault failure is reported to the model as unavailable and never allowed to end the turn.
...
**`estimation_techniques.py`** and **`technique_prompts.py`** are the five estimation techniques the user picks between. The rule is the one the rest of the product runs on: **the squad votes, the code counts** — no schema a seat answers contains a story-point field. Nine seats own an exclusive slice of the sixteen factors (a test asserts full, non-overlapping coverage), and the final scorecard takes each factor from its owner rather than averaging, because averaging the frontend engineer's guess at the migration with the data engineer's knowledge of it produces a number neither would defend.

Two rules there were each written after getting it wrong. A Planning Poker **card** is the story sized as if all of it were as demanding as that seat's part — the obvious implementation, laying a seat's one-to-three scores over the baseline, moves the total by a few points, never crosses a band edge, and returns an identical card from every seat: spread always zero, second round never fires, and what is left is a scoring pass in Planning Poker's clothes. And a bucket anchor the model names but that is **not on the list is refused, never substituted** — quietly picking a plausible replacement produces a confident number derived from a reference nobody chose, which is the repository path check's failure in a different costume.

The techniques must stay genuinely different in cost, precision and the answer they can reach; five code paths converging on one function make the picker decoration. `seat_scores()` reads whichever shape a seat actually returned (a list of objects, or a bare factor→score map) because rejecting a usable shape throws away minutes of CPU for nothing.
...
Every path that accepts bytes from a client carries a ceiling. HTTP uploads had one; the Talk WebSocket's audio buffer and the request body did not, and the audio path is reachable by leaving a microphone running rather than by anything adversarial. An oversized recording is **refused with a message**, never truncated — half a sentence transcribed and answered confidently is worse than a turn that did not happen.
