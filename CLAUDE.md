...
**`yukti.py`** holds the persona, the capability register and the voice shaping. The register is the load-bearing part and it is **data, not prose**: each faculty records whether it is actually wired, and an unwired one carries the exact sentence YUKTI says instead. Five are wired (second brain, memory, web research, documents, animation); four are not (screen vision, calendar, email, model swapping). `gate` refuses those **in code, before generation** — a prompt telling a 1B model not to describe a screen it cannot see still lets it describe one perhaps once in five, and spoken aloud there is no URL to click and no panel to open, so the listener has nothing to check it against. Do not add a faculty to the persona without wiring it.
